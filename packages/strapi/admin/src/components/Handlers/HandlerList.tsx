...
*/
import { Flex, Icon, Link, LinkButton, Stack, Td, Tooltip, Tr, Typography,} from '@strapi/design-system';

import {Pencil, Dot, Eye} from '@strapi/icons';
...
const HandlerList = ({item}: HandlerListItemProps): JSX.Element => {
    const {setVisible, visible, handler, setHandler} = useContext(HandlerContext);
    ...
    function setActiveState(item: HandlerType) {
        ...
        handlerRequests.toggleStatus(item.id, status).then((response) => {
            ...
            <Td>
                <Flex justifyContent="right" alignItems="right">
                    <Stack horizontal={true} spacing={3}>
                        <Tooltip description={formatMessage({id:"webforms.tooltips.enabled"})}>
                            <Link onClick={() => setActiveState(item)} to="#">
                                <Icon
                                    aria-hidden={true}
                                    colors={(theme) => ({
                                        rect: {
                                            fill: active ? theme.colors['success600'] : theme.colors['danger600'],
                                        },
                                    })}
                                    as={Dot}
                                />
                        ...
                        <LinkButton startIcon={<Eye/>} onClick={() => toggle(!visible, item)}>
